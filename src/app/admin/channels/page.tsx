        ...
        <div className="flex items-center justify-between">
           <div>
              <h1 className="text-[18px] font-bold flex items-center gap-2 text-primary-900">
                 <GitBranch className="text-primary-600" size={20} /> Acquisition Channels
              </h1>
              <p className="text-[12px] text-muted-foreground mt-0.5">Define the hierarchical source tree for lead registration.</p>
           </div>
        </div>

        <div className="flex flex-col lg:flex-row h-full lg:h-[calc(100vh-220px)] border rounded-md overflow-hidden bg-card border-primary-100 shadow-sm">
           {/* Main Channels List */}
           <div className={cn(
             ...
             selectedMainId && "hidden lg:flex"
              ...
              <div className="p-3 border-b flex items-center justify-between px-4">
                 ...
                 <Badge variant="outline" className="text-[10px] h-4.5 bg-white border-primary-100 text-primary-600">{mainChannels.length}</Badge>
              </div>
              
              <div className="flex flex-col flex-1 overflow-hidden">
                 <form onSubmit={handleAddMain} className="p-3 border-b bg-white flex gap-2">
                    <Input placeholder="Add main category..." className="h-8 text-[12px] bg-primary-50/30" value={newChannelName} onChange={(e) => setNewChannelName(e.target.value)} />
                    <Button size="icon" className="h-8 w-8 shrink-0 bg-primary-600 hover:bg-primary-700"><Plus size={16} /></Button>
                 </form>
                 
                 <div className="flex-1 overflow-y-auto">
                    {loading ? (
                      <div className="p-10 text-center"><Loader2 className="animate-spin mx-auto text-primary-200" size={24} /></div>
                      ...
                      <div 
                        ...
                        className={cn(
                          ...
                          selectedMainId === item.id ? "bg-white border-r-4 border-r-primary-600 shadow-inner" : "",
                      ...
                      >
                         ...
                         <div className="flex items-center gap-1.5 ml-2 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                            <Switch className="scale-75 data-[state=checked]:bg-primary-600" checked={item.active} onCheckedChange={() => handleToggleActive(item.id, item.active)} onClick={e => e.stopPropagation()} />
                            <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-300 hover:text-primary-600" onClick={(e) => { e.stopPropagation(); handleStartEdit(item); }}>
                               <Edit2 size={14} />
                            </Button>
                            <TooltipProvider>
                               <Tooltip>
              ...
              {selectedMainId ? (
                 <>
                    <div className="p-3 border-b flex items-center justify-between lg:justify-start gap-4 px-4 bg-slate-50/50">
                       <Button variant="ghost" size="sm" className="lg:hidden h-8 gap-2 text-[12px] font-bold text-primary-700" onClick={() => setSelectedMainId(null)}>
                          <ChevronLeft size={16} /> All Sources
                       </Button>
                       <h2 className="text-[11px] font-bold uppercase text-slate-500 tracking-widest truncate">
                          Sub-channels for <span className="text-primary-700">{mainChannels.find(c => c.id === selectedMainId)?.name}</span>
                       </h2>
                    </div>

                    <div className="flex flex-col flex-1 overflow-hidden">
                       <form onSubmit={handleAddSub} className="p-4 border-b flex gap-3">
                          <Input placeholder="Add granular detail (e.g. TikTok)..." className="h-9 text-[13px] border-primary-50" value={newSubChannelName} onChange={(e) => setNewSubChannelName(e.target.value)} />
                          <Button size="sm" className="h-9 px-6 bg-primary-600 hover:bg-primary-700 font-bold uppercase text-[11px] shrink-0">Link Sub-Source</Button>
                       </form>
                       <div className="flex-1 overflow-y-auto">
                          {subChannels.map(item => (
                             <div 
                               key={item.id} 
                               className={cn(
                                 "group p-3 px-6 border-b flex items-center justify-between transition-all hover:bg-primary-50/20",
                                 !item.active && "opacity-50"
                               )}
                             >
                                <div className="flex items-center gap-3 flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-1.5 ml-2 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                                   <Switch className="scale-75 data-[state=checked]:bg-primary-600" checked={item.active} onCheckedChange={() => handleToggleActive(item.id, item.active)} />
                                   <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-300 hover:text-primary-600" onClick={() => handleStartEdit(item)}>
                                      <Edit2 size={14} />
                                   </Button>
                                   <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-200 hover:text-red-500 disabled:opacity-20" disabled={(item.usageCount || 0) > 0} onClick={() => handleDelete(item.id, item.usageCount)}>
                                      <Trash2 size={14} />
