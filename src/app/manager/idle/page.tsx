      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 text-[11px] text-slate-400">
             ...
             <Button variant="outline" size="sm" className="h-8 h-8" onClick={() => setLastRefreshed(new Date())}>Refresh</Button>
          </div>
        </div>

        {/* Summary Strip */}
        <div className="bg-slate-50 border p-3 rounded-md flex flex-wrap gap-6 items-center shadow-sm">
           <div className="flex items-center gap-2">
             <span className="text-[20px] font-bold">{stats.total}</span>
             <span className="text-[12px] font-medium text-slate-500 uppercase tracking-tight">Idle Leads</span>
           </div>
           <div className="h-4 w-px bg-slate-200 hidden md:block" />
           <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-red-500" />
              <span className="text-[12px] font-bold">{stats.critical} Critical (240h+)</span>
           </div>
           <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-orange-500" />
              <span className="text-[12px] font-bold">{stats.warning} Warning (120h+)</span>
           </div>
           <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-amber-500" />
              <span className="text-[12px] font-bold">{stats.approaching} Approaching</span>
           </div>
        </div>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y">
                  {idleLeads.map((lead) => {
                    ...
                    const severityColor = lead.idleHours >= 240 ? "bg-red-50/50" : lead.idleHours >= 120 ? "bg-orange-50/50" : "bg-amber-50/30";
                    ...
                    return (
                      <React.Fragment key={lead.id}>
                        <tr className={cn("h-10 transition-colors group", severityColor)}>
                          <td className="px-3">
                            <Link href={`/leads/${lead.id}`} className="font-bold hover:underline truncate">
                              {lead.clientName}
                            </Link>
                          </td>
                          <td>
                          ...
                          <td className="px-3 text-right">
                             <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                               <Button variant="ghost" size="sm" className="h-6 text-[11px] gap-1" onClick={() => setExpandedLeadId(expandedLeadId === lead.id ? null : lead.id)}>
                                 {expandedLeadId === lead.id ? <ChevronUp size={12} /> : <ChevronDown size={12} />} Log
                               </Button>
                               <Button variant="ghost" size="sm" className="h-6 text-[11px] text-slate-500" onClick={() => markDormant(lead.id)}>
                            ...
                            <td colSpan={6} className="p-4 px-10">
                              <div className="flex gap-4">
                                <div className="flex-1 space-y-2">
                                  <div className="flex items-center gap-4">
                                     <Select value={quickType} onValueChange={(val) => setQuickType(val as ActivityType)}>
                                       <SelectTrigger className="h-8 w-[160px] text-[11px] bg-white">
                                         <SelectValue />
                                       </SelectTrigger>
                                       <SelectContent>
                                     ...
                                     <p className="text-[11px] text-muted-foreground italic">Add a quick note to reset the idle timer for this lead.</p>
                                  </div>
                                  <Textarea 
                                    className="bg-white text-[12px] min-h-[60px] resize-none" 
                                ...
                                <div className="flex flex-col gap-2 justify-end">
                                  <Button size="sm" className="h-8 bg-cyan-600 hover:bg-cyan-700 text-[12px]" onClick={() => handleQuickAdd(lead)} disabled={isSubmitting || !quickRemark.trim()}>
                                    {isSubmitting ? <Loader2 className="animate-spin" size={14} /> : 'Save Update'}
                                  </Button>
