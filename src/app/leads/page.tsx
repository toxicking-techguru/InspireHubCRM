...
export default function LeadsPage() {
  ...
  // Filtering logic for the mock leads
  const filteredLeads = LEADS.filter(lead => {
    const isOwner = user?.role === 'Agent' ? lead.agent_id === user.id : true;
                  ...
                  <tr key={lead.id} className="group">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-col">
                        <span className="font-bold text-sm">{lead.client_name}</span>
                        <div className="flex items-center gap-3 mt-1">
                          <span className="flex items-center gap-1 text-[11px] text-muted-foreground">
                            <Mail size={10} /> {lead.client_email}
                          </span>
                          <span className="flex items-center gap-1 text-[11px] text-muted-foreground">
                            <Phone size={10} /> {lead.client_phone}
                          </span>
                        </div>
                      </div>
                    </td>
                    <td>
                      <div className="flex flex-col">
                        <span className="text-sm">{PRODUCTS.find(p => p.id === lead.product_id)?.name}</span>
                        <span className="flex items-center gap-1 text-[10px] text-muted-foreground">
                          <Globe size={10} /> {lead.business_country}
                        </span>
                      </div>
                    </td>
