...
import { Plus, Trash2, Pin, Bell, Loader2, StickyNote } from 'lucide-react';
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-[350px] p-0 flex flex-col">
        ...
        <div className="p-4 space-y-4 border-b bg-white">
           ...
           />
           <Button className="w-full h-9 gap-2 font-bold uppercase text-[11px] bg-primary hover:bg-primary/90" disabled={isAdding || !newContent.trim()} onClick={handleAdd}>
        ...
        <div className="flex-1 overflow-auto p-4 space-y-3 bg-slate-50/50">
           ...
           {notes?.length === 0 && !loading && (
             <div className="py-20 text-center opacity-40">
               <p className="text-[12px] italic">No active notes. Use the box above to capture thoughts.</p>
             </div>
           )}
        </div>
      </SheetContent>
